...
export const print = (order) => {
	...
	switch(Number(process.env.PRINTER_TYPE)){
		case 1:
			...
			break
		case 2:
			device = new Serial(port, { baudRate: 115200, stopBit: 1 })
			break
		case 3:
		...
		default:
			...
			console.log(process.env.PRINTER_TYPE)
	}
	...
	device.open(async function(err) {
		...
		for(let i=0;i<order.length;i++) {
			...
			printer
				...
				.text(` `)
				.text(`PEDIDO: ${order[i].itens.replace(regexEmoji, '')}`)
				.text(`${order[i].itensComp.replace(regexEmoji, '')}`)
